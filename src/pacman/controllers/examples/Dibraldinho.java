...
/*
 * @autor Rafael Tenfen e Douglas Felipe Bussmann
 */
public class Dibraldinho extends Controller<MOVE> {

	private static final int SAFE_DISTANCE      = 16;
...
//		}
		...
		nodoList = new ArrayList<Dibraldinho.Nodo>();
		nodoListSomething = new ArrayList<Dibraldinho.Nodo>();
...
//		System.out.println("FirstNodo and is LArgura:" + this.isBreadthSearch + " points " + points);
	...
	public Nodo getMaxPointsLastNodoList(ArrayList<Nodo> list, Game game) {
		Nodo oNodo = null;
		float points = 0;
				...
				if (!this.isIndexInFatherTree(i, aux)) {
						...
						if (i == p) {
							...
							for (GHOST g : ghosts) {
								...
								if (game.getGhostLairTime(g) <= 0) {
									if (i == ghostIndex ||  game.getDistance(i, ghostIndex, this.lastMove, DM.PATH) < (Dibraldinho.SAFE_DISTANCE - 10)) {
										nodoPoints = 0;
										isSafe = false;
									}
								} 
					...
					for (int pp : powerPills) {
						if (i == pp && !this.haveGhostInLair(game) && this.ghostsEdible.size() == 0) {
							nodoPoints += Dibraldinho.POWER_PILL_POINTS;
					...
					for (GHOST g : ghosts) {
						int ghostIndex = game.getGhostCurrentNodeIndex(g);
						if (game.getGhostLairTime(g) <= 0) {
							if (i == ghostIndex ||  game.getDistance(i, ghostIndex, game.getMoveToMakeToReachDirectNeighbour(i, ghostIndex), DM.MANHATTAN) < Dibraldinho.SAFE_DISTANCE) {
...
//								System.out.println("Ghost is too close " + g.toString());
					...
					for (GHOST ge : ghostsEdible) {
						...
						if (game.getDistance(i, ghostIndex, DM.EUCLID) < Dibraldinho.AGRESSIVE_DISTANCE) {
							boolean safe = true;
...
//								}
							...
							if (safe) {
								if (game.getGhostEdibleTime(ge) > this.SAFE_EDIBLE_TIME) {
									this.nodoList.add(new Nodo(ghostIndex, null, (nodoPoints + Dibraldinho.GHOSTS_POINTS), this.MAX_NIVEL_NODO));
								} else {
									nodoPoints = 0;
									isSafe = false;
								}		
...
//						newNodo.setPai(null);
		...
		if (oNodo == null) {
			oColor = Color.PINK;
			oNodo = this.getMaxPointsLastNodoList(this.nodoList, game);
		}
...
//			System.out.println("Nivel NODO" + oNodo.getNivel());
			GameView.addPoints(game, oColor, game.getShortestPath(myPosition, oNodo.getIndex(), game.getPacmanLastMoveMade()));
...
//			System.out.println("NODO � null");
		...
		if (this.MAX_NIVEL_NODO < 120 && this.MAX_NIVEL_NODO > 0) {
		...
		// vai atras da p�lula por um caminho seguro
